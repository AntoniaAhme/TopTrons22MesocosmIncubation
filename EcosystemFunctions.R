...
## Create a table for statistical outout of two-way RM anova of residuals
Parameter <- rep(c("Biomass", "GOP", "C:N", "C:P"), each=3)
Effect <- rep(c("Temperature","Time","Temperature:Time"), times=4)
stat <- data.frame(Parameter, Effect)
stat['DFn'] <- NA
...
## Create residuals of Biomass dataset
pan1 <- pan %>% select(µgL_c, temp, day, plankto_ID)
pan1$uniID <- paste(pan1$temp, pan1$day)

pan0 <- pan1 %>% 
  group_by(temp, day) %>%
  dplyr::summarise(mean = mean(µgL_c))
pan0$uniID <- paste(pan0$temp, pan0$day)
pan1 <- left_join(pan1, pan0)

pan1$residuals <- abs(pan1$µgL_c-pan1$mean)

# Remove attributes of mutate
pan2 <- as.matrix(pan1)
pan2 <- as.data.frame(pan2)
pan2$day <- as.factor(pan2$day)
pan2$temp <- as.factor(pan2$temp)
pan2$plankto_ID <- as.factor(pan2$plankto_ID)
pan2$residuals <- as.numeric(pan2$residuals)

# Summary
pan2 %>%
  group_by(temp, day) %>%
  get_summary_stats(residuals, type = "mean_sd")

bxp <- ggboxplot(
  pan2, x = "day", y = "residuals",
  color = "temp", palette = temp_pal)
bxp
...
ggqqplot(pan2, "residuals", ggtheme = theme_bw()) +
  facet_grid(day ~ temp, labeller = "label_both")
# very few datapoints, but apparently normal

# Compute 2-way RM ANOVA
res.aov <- anova_test(
  data = pan2, dv = residuals, wid = plankto_ID, 
  between = temp, within = day)
res.aov
# sphericity not violated
...
## Create residuals of GPP dataset
pan1 <- oxy %>% select(o2_poc, temp, day, plankto_ID)
pan1$uniID <- paste(pan1$temp, pan1$day)

pan0 <- pan1 %>% 
  group_by(temp, day) %>%
  dplyr::summarise(mean = mean(o2_poc))
pan0$uniID <- paste(pan0$temp, pan0$day)
pan1 <- left_join(pan1, pan0)

pan1$residuals <- abs(pan1$o2_poc-pan1$mean)
...
## Create residuals of CN dataset
pan1 <- pan %>% select(cn, temp, day, plankto_ID)
pan1$uniID <- paste(pan1$temp, pan1$day)

pan0 <- pan1 %>% 
  group_by(temp, day) %>%
  dplyr::summarise(mean = mean(cn))
pan0$uniID <- paste(pan0$temp, pan0$day)
pan1 <- left_join(pan1, pan0)

pan1$residuals <- abs(pan1$cn-pan1$mean)
...
res.aov_cn <- get_anova_table(res.aov)
# main effect of day and interaction

# main effect of day
one.way <- pan2 %>%
  group_by(temp) %>%
  anova_test(dv = residuals, wid = plankto_ID, within = day) %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
one.way
# there is a significant main effect of day within 6 degree

# add main results to stat table
stat[7:9,3:6] <- res.aov_cn[,2:5]

## Create residuals of CP dataset
pan1 <- pan %>% select(cp, temp, day, plankto_ID)
pan1$uniID <- paste(pan1$temp, pan1$day)

pan0 <- pan1 %>% 
  group_by(temp, day) %>%
  dplyr::summarise(mean = mean(cp))
pan0$uniID <- paste(pan0$temp, pan0$day)
pan1 <- left_join(pan1, pan0)

pan1$residuals <- abs(pan1$cp-pan1$mean)
...
res.aov_cp <- get_anova_table(res.aov)
# nsignificant main effect of day 

# add main results to stat table
stat[10:12,3:6] <- res.aov_cp[,2:5]

# Create data table of results
write_xlsx(stat, "Data/StatisticsVar_6-12.xlsx")
...
res.aov_bm <- get_anova_table(res.aov)
# main effects of temp and time and interaction

# Considering a significant interaction
# main effect of temp
one.way <- pan2 %>%
  group_by(day) %>%
  anova_test(dv = residuals, wid = plankto_ID, between = temp) %>%
  get_anova_table() %>%
  adjust_pvalue(method = "bonferroni")
one.way
# there is a significant main effect on the last three days

# pairwise comparisons
pwc <- pan2 %>%
  group_by(day) %>%
  pairwise_t_test(
    residuals ~ temp, paired = FALSE,
    p.adjust.method = "bonferroni"
  )
print(pwc, n=30)
# significant main effect on last three days
...
one.way <- pan2 %>%
  ...
  adjust_pvalue(method = "bonferroni")
one.way
# there is a significant main effect within 18 degree
...
res.aov_cp <- get_anova_table(res.aov)
# effect of day and interaction with time
...
# Create data table
write_xlsx(stat, "Data/StatisticsVar_6-18.xlsx")
