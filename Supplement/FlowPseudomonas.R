#### TopTrons Flow Cytometry Data - Pseudomonas like bacteria ####
...
count <- plyr::rename(count,  c("S_HFL-4" = 'PseudoAbundance'))


#### PLOT BARPLOT OF PSEUDOMONAS PER MESOCOSM OVER TIME ####
pseudo <- ggplot(count, aes(day, PseudoAbundance, col=plankto_ID)) + 
  geom_point(size = 3) + 
  geom_line(size = 0.75) +
...
pseudo
ggsave("Output/PseudomonasLike.png", pseudo, height = 5, width = 9, dpi = 320)
